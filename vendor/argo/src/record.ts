...
/**
 * Resolve Playwright's JS CLI so it can be spawned with `node` directly.
 *
 * Spawning `npx` is not portable: on Windows it is a `.cmd` shim, which
 * execFile reports as ENOENT, and since Node's CVE-2024-27980 fix it reports
 * EINVAL even when given the `.cmd` extension. Running the CLI entrypoint
 * under `process.execPath` sidesteps shell resolution entirely.
 */
function playwrightCommand(): { cmd: string; prefix: string[] } {
  try {
    return { cmd: process.execPath, prefix: [requireFrom.resolve('@playwright/test/cli')] };
  } catch {
    return { cmd: process.platform === 'win32' ? 'npx.cmd' : 'npx', prefix: ['playwright'] };
  }
}
...
export async function record(demoName: string, options: RecordOptions): Promise<RecordResult> {
  ...
  try {
    return await new Promise<RecordResult>((resolve, reject) => {
      ...
      execFile(pw.cmd, [...pw.prefix, 'test', '--config', recordConfigPath, '--grep', demoName, '--project', 'demos'], {
        env: {
          ...process.env,
