...
async function getTranscriber(model: string): Promise<Transcriber> {
  ...
  // Imported lazily. Transcription is opt-in (`tts.transcribe`), but
  // `generate.ts` imports this module unconditionally — a static import here
  // made the whole TTS path fail to load when the optional ONNX dependency
  // isn't installed, which is the default for cloud TTS engines.
  let pipeline: (task: string, model: string) => Promise<unknown>;
  try {
    ({ pipeline } = await import('@huggingface/transformers'));
  } catch {
    throw new Error(
      "Word-level transcription requires the '@huggingface/transformers' package. " +
      'Install it with: npm install @huggingface/transformers — ' +
      'or leave `tts.transcribe` unset, which is the default.'
    );
  }
